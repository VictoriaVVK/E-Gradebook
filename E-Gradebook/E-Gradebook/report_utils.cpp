...
void averageGradeAllSubjects(const vector<Grade>& grades, const vector<Student>& students) {
    ...
    if (count > 0) {
        cout << " Overall average grade: " << fixed << setprecision(2) << (sum / count) << endl;
    }
    else {
        cout << " No grades found for that student.\n";
    }
}


void averageGradePerSubject(const vector<Grade>& grades, const vector<Student>& students, const vector<Subject>& subjects) {
    cout << "\n--- Students ---\n";
    for (const auto& s : students) {
        cout << "ID: " << s.id << " | Name: " << s.fullName << "\n";
    }
